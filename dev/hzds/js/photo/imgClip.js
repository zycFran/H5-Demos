...
ImgClip.prototype = {
	init: function(opt) { // 初始化
		...
		this.winSize = { w: view().w, h: view().h};

		//opt.cutW == 'winW' ? (this.cutW = this.winSize.w) : (this.cutW = Number(opt.cutW));
		opt.cutW = this.winSize.w * 0.9;

		var hh = parseInt(opt.cutW * this.cutScale);
		this.cutSize = {
			w: opt.cutW,
			h: hh,
			t: (this.winSize.h - hh)/2,
			l: (this.winSize.w - opt.cutW)/2
		};
